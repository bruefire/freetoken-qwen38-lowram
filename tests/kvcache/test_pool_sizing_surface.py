...
def test_resolve_pool_class_follows_attn_type():
    ...
    cases = [
        ...
        ((_spec("full", AttnType.QSA, index_head_dim=128),), QSAKVCache),
    ]
...
def test_factory_builds_qsa_pool_with_sparse_layer_remap(monkeypatch):
    import dataclasses
    ...
    model_config.kv_cache_group_specs = lambda: (
        _spec("full", AttnType.QSA, index_head_dim=128),
    )
    # Mirror Qwen's single sparse layer rather than the helper's default (0, 1).
    spec = model_config.kv_cache_group_specs()[0]
    model_config.kv_cache_group_specs = lambda: (
        dataclasses.replace(spec, layer_ids=(3,), num_index_layers=1),
    )
    ...
    assert pool.k_cache(3).shape == (2, 16, 2, 64)
    assert pool.index_k_cache(0).shape == (32, 128)
