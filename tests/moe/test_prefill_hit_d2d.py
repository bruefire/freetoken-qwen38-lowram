...
def test_batch_memcpy_roundtrip():
    ...
    # The copy is enqueued on a fresh stream, so avoid an unrelated default-
    # stream initialization that could race it.
    dst = torch.empty(rows, feat, dtype=torch.uint8, device="cuda")
