...
def parse_args(
    ...
    parser.add_argument(
        "--decode-profile-interval",
        type=_nonnegative_int,
        default=ServerArgs.decode_profile_interval,
        help=(
            "Profile the live scheduler and log one FT.DecodeProfile JSON aggregate every "
            "N completed decode forwards; 0 disables profiling (default)."
        ),
    )
    ...
    parser.add_argument(
        ...
        help="Collect and periodically log decode MoE expert-cache hit/miss statistics.",
    )
