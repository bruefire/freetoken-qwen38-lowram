...
from freetoken.utils import configure_decode_profile_ranges, init_logger
...
def _decode_profile_requested(argv: list[str]) -> bool:
    """Read only the opt-in bit before importing scheduler/model hot-path modules."""
    for index, arg in enumerate(argv):
        if arg.startswith("--decode-profile-interval="):
            value = arg.partition("=")[2]
        elif arg == "--decode-profile-interval" and index + 1 < len(argv):
            value = argv[index + 1]
        else:
            continue
        try:
            ...
            # argparse will issue the authoritative error; enabling ranges on this doomed
            # startup is harmless and avoids importing hot paths in the disabled form first.
            return True
    return False
...
def launch_server(
    ...
    if _decode_profile_requested(raw_argv):
        # spawn inherits this for every TP scheduler. Configure this interpreter too,
        # before args.py imports freetoken.scheduler (whose modules bind the range symbol).
