...
@dataclass
class HostInputIds:
    """CPU token views aligned with a decode batch's padded requests.

    ``ready`` protects views backed by the sampler's asynchronous D2H ring.  A
    host-side model input can consume that transfer directly instead of copying
    the same token back from the GPU token pool on the following step.
    """
...
class Batch:
    ...
    # Decode-only CPU counterpart of ``input_ids``, one exact view per padded
    # request.  The scheduler fills it only when every row is already available.
    host_input_ids: HostInputIds | None = field(default=None, init=False)
