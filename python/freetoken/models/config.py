...
class FullAttentionGroupConfig(BaseAttentionGroupConfig):
    ...
    index_head_dim: int = 0
    num_index_layers: int = 0
    # Most full-family groups derive FULL/MLA/DSA/BSA from the fields above.
    # Architectures with distinct GQA sparse semantics (Qwen4 QSA) pin their
    # backend type explicitly while retaining the common paged-KV group shape.
    attn_type: AttnType | None = None
...
def _full_group_attn_type(group: FullAttentionGroupConfig) -> AttnType:
    ...
    if group.attn_type is not None:
        return group.attn_type
    if not group.mla:
