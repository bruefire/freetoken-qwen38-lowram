...
def _gdn_fast_path_enabled() -> bool:
    return os.getenv("FREETOKEN_GDN_FAST_PATH", "1").strip().lower() not in _FALSE_VALUES
...
class _GatedRMSNorm(BaseOP):
    ...
    def __init__(self, dim: int, eps: float):
        ...
        self.eps = eps
        self.save_stats = not _gdn_fast_path_enabled()
...
class Qwen3_5GatedDeltaNet(BaseOP):
    ...
    def _save_mtp_prefix_state(
        ...
        if (
            ...
            or self._mtp_prefix_conv_state.shape != conv_shape
            or self._mtp_prefix_conv_state.device != conv_state.device
            or self._mtp_prefix_conv_state.dtype != conv_state.dtype
        ):
            self._mtp_prefix_conv_state = torch.empty(
                conv_shape, dtype=conv_state.dtype, device=conv_state.device
            )
        if (
            self._mtp_prefix_recurrent_state is None
            or self._mtp_prefix_recurrent_state.shape != recurrent_shape
            or self._mtp_prefix_recurrent_state.device != recurrent_state.device
            or self._mtp_prefix_recurrent_state.dtype != recurrent_state.dtype
        ):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        with decode_profile_range("FT.GDN"):
    ...
    def _forward_impl(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if self._fp8:
            with decode_profile_range("FT.GDN.InProj"):
        ...
        if getattr(batch, "mtp_verify", False):
            rows = []
            with decode_profile_range("FT.GDN.Conv1d"), decode_profile_range(
                "FT.GDN.Recurrence"
            ):
                for row in range(total):
                    ...
                    v = vf.reshape(
                        1, 1, self.num_v_heads, self.head_v_dim
                    ).to(dtype)
                    rows.append(
                        gdn_decode_fla(
                            q,
            ...
            with decode_profile_range("FT.GDN.Conv1d"):
                mixed = self._conv_decode(conv_in, fla.cache_indices, pool)  # [B, conv_dim]
            with decode_profile_range("FT.GDN.Recurrence"):
                B = mixed.shape[0]
                qf, kf, vf = torch.split(
                    mixed, [self.key_dim, self.key_dim, self.value_dim], dim=-1
                )
                ...
                core_out = gdn_decode_fla(
                    ...
                    cu_seqlens=fla.cu_seqlens, scale=self.scale,
                )
        else:
            with decode_profile_range("FT.GDN.Conv1d"):
