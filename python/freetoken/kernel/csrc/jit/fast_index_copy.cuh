...
__always_inline __device__ auto load_vec(const void* __restrict__ src) {
    ...
    constexpr bool kNeedsBoundsCheck = kBytes % kBytesPerLoop != 0;
    ...
    if constexpr (!kNeedsBoundsCheck) {
        // Preserve the original aligned fast path verbatim: exact trip count,
        // complete unrolling, and no per-package predicate in generated code.
...
__always_inline __device__ void store_vec(void* __restrict__ dst, const Tp& vec) {
    ...
    if constexpr (!kNeedsBoundsCheck) {
        // Keep the aligned store identical to the original unconditional path.
        constexpr auto kLoopCount = kBytes / kBytesPerLoop;
