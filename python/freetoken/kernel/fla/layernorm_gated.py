...
def _layer_norm_fwd_1pass_kernel(
    ...
    if SAVE_STATS:
        # Training callers consume these statistics. FreeToken inference callers
        # can omit both this global-memory write and the temporary allocation.
...
def _layer_norm_fwd(
    ...
    mean = (
        ...
        if save_stats and not is_rms_norm
        else None
    )
    rstd = (
        torch.empty((ngroups * M,), dtype=torch.float32, device=x.device)
    ...
    with _device_context(x.device):
        _layer_norm_fwd_1pass_kernel[grid](
            ...
            bias,
            z,
            mean if mean is not None else out,
            rstd if rstd is not None else out,
            x.stride(0),
            out.stride(0),
