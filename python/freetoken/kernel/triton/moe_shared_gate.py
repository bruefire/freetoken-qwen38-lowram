...
"""Shared-expert gate reduction and gated add kernels."""
...
def _gate_sigmoid_kernel(
    ...
    hidden = tl.load(hidden_ptr + row * stride_h + offsets, mask=mask, other=0.0).to(tl.float32)

    if launch_pdl:
        tl.extra.cuda.gdc_launch_dependents()

    tl.store(gate_ptr + row, tl.sigmoid(tl.sum(hidden * weight, axis=0)))


def shared_gate_sigmoid(hidden_states: torch.Tensor, gate_weight: torch.Tensor) -> torch.Tensor:
    """Return sigmoid(hidden_states @ gate_weight) as fp32."""
...
def _gate_mul_add_kernel(
    ...
    routed = tl.load(routed_ptr + row * stride_r + offsets, mask=mask, other=0.0).to(tl.float32)
    shared = tl.load(shared_ptr + row * stride_s + offsets, mask=mask, other=0.0).to(tl.float32)

    if launch_pdl:
        tl.extra.cuda.gdc_launch_dependents()

    tl.store(out_ptr + row * stride_o + offsets, routed + gate * shared, mask=mask)


def shared_gate_mul_add(
    routed: torch.Tensor, shared: torch.Tensor, gate: torch.Tensor
) -> torch.Tensor:
    """Return routed + gate[:, None] * shared."""
