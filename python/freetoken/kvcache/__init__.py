...
def create_kvcache_pool(
    ...
    # Indexed GQA sparse attention (MiniMax-M3 BSA / Qwen4 QSA): one full-attention
    # group carrying the index dims with mla=False -> MHA plus an index-key slab.
    # The same spec fields drive the KV cost model and live pool allocation.
    from freetoken.attention import AttnType as _AttnType

    if len(kv_specs) == 1 and kv_specs[0].attn_type in {
        _AttnType.BSA,
        _AttnType.QSA,
    }:
        from .bsa_pool import BSAKVCache
        ...
        spec = kv_specs[0]
        pool_cls = QSAKVCache if spec.attn_type == _AttnType.QSA else BSAKVCache
        pool_num_layers = max(
            model_config.num_layers,
            max(spec.layer_ids, default=-1) + 1,
        )
        return pool_cls(
            num_kv_heads=spec.num_kv_heads,
