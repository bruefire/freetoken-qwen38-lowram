...
from freetoken.utils import decode_profile_range, div_even
...
class OffloadMoELayer(MoELayer):
    ...
    def forward(
        ...
        ctx = get_global_ctx()
        if ctx.batch.is_prefill and not getattr(
            ctx.batch, "moe_decode_cache", False
        ):
    ...
    def decode_forward(
        ...
        with decode_profile_range("FT.MoE.RouterTopK"):
            topk_weights, topk_ids = fused_topk(
                hidden_states=hidden_states,
                gating_output=router_logits,
                topk=self.top_k,
    ...
    def _decode_routed(
        ...
        if cache.is_cpu_layer(self.layer_id):
            ...
            with decode_profile_range("FT.MoE.CPULayer"):
                return executor.decode(self.layer_id, hidden_states, topk_weights, topk_ids)
        if cache.decode_target == "hybrid":
            return self._decode_hybrid(cache, hidden_states, topk_weights, topk_ids)
        with decode_profile_range("FT.MoE.CacheLookup"):
            cache.ensure_experts(self.layer_id, topk_ids)
        with decode_profile_range("FT.MoE.ExpertFetch"):
            cache.copy_missing()
        with decode_profile_range("FT.MoE.ExpertGEMM"):
    ...
    def _decode_hybrid(
        ...
        with decode_profile_range("FT.MoE.CacheLookup"):
            cache.ensure_experts_hybrid(self.layer_id, topk_ids)  # -> slot/fetched or -1
