...
class TritonAttentionBackend(BaseAttnBackend):
    ...
    def prepare_metadata(self, batch: Batch) -> None:
        ...
        with decode_profile_range("FT.AttentionMetadata.Shapes"):
        ...
        with decode_profile_range("FT.AttentionMetadata.TensorBuild"):
            # Construct list-derived metadata in pinned host memory, then enqueue H2D.
            # torch.tensor(list, device="cuda") uses a blocking pageable copy; under
            # overlap scheduling its cudaStreamSynchronize waits for the preceding engine
            # forward and makes that wait look like prepare_metadata CPU work.
            cpu_kwargs = {
                "dtype": torch.int32,
                "device": "cpu",
                "pin_memory": device.type == "cuda" and torch.cuda.is_available(),
            }
            prefix_lens = torch.tensor(cached_lens, **cpu_kwargs).to(
                device, non_blocking=True
            )
            indptr = torch.tensor([0] + seqlens_k, **cpu_kwargs).cumsum_(0).to(
                device, non_blocking=True
            )
            if is_decode:
                ...
                cu_seqlens_q_gpu = (
                    torch.tensor([0] + seqlens_q, **cpu_kwargs)
                    .cumsum_(0)
                    .to(device, non_blocking=True)
                )
        with decode_profile_range("FT.AttentionMetadata.PageTableGather"):
            indices = torch.cat(
        ...
        if getattr(self.kvcache, "swa_paged", False):
            ...
            with decode_profile_range("FT.AttentionMetadata.SWATranslate"):
                swa_indices = self.kvcache.translate_loc_from_full_to_swa(indices)

        with decode_profile_range("FT.AttentionMetadata.QueryMapping"):
