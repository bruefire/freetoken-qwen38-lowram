...
class Engine:
    def __init__(self, config: EngineConfig):
        ...
        # launched.  A two-slot pinned ring removes the per-step D2H allocation
        # without letting a later sample overwrite an in-flight/undrained view.
        sample_capacity = config.max_running_req * (
            3 if getattr(self.model, "mtp", None) is not None else 1
        )
    ...
    def forward_batch(self, batch: Batch, args: BatchSamplingArgs) -> ForwardOutput:
        ...
        with decode_profile_range(f"FT.{phase}Step"):
    ...
    def _forward_batch_impl(self, batch: Batch, args: BatchSamplingArgs) -> ForwardOutput:
        ...
        with self.ctx.forward_batch(batch):
            if self.graph_runner.can_use_cuda_graph(batch):
                ...
                if batch.is_decode and getattr(
                    self, "_decode_profile_stage_graph_inputs", False
                ):
                    # The decode profiler's eager detail pass deliberately keeps QSA's
                    # captured fixed-width row/score shapes.  Otherwise eager QSA sizes to
                    # the live context and understates the graph's top-k/score work.
                    with decode_profile_range("FT.Eager.GraphShapeInputStaging"):
                        self.attn_backend.prepare_for_replay(batch)
                with decode_profile_range("FT.Eager.ModelForward"):
                    if mtp_prefill:
        ...
        with decode_profile_range("FT.SamplerAndD2H"):
...
def _adjust_config(config: EngineConfig):
    ...
    if (
        ...
        # Reject at config time: the indexed-GQA pools' own assert only fires
        # after the model is resident (and not at all under `python -O`).
        raise ValueError(
