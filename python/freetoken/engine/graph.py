...
from freetoken.utils import decode_profile_range, init_logger, mem_GB
...
class GraphRunner:
    ...
    def _capture_graphs(self, max_seq_len: int, vocab_size: int, model: BaseLLMModel):
        ...
        for bs in pbar:
            ...
            prepare_model_inputs = getattr(model, "prepare_cuda_graph_capture", None)
    ...
    def replay(
        ...
        with decode_profile_range("FT.Graph.PLEPrepare"):
            self.model.prepare_cuda_graph_replay(batch)
        with decode_profile_range("FT.Graph.InputStaging"):
            self.buffer.copy_from(batch)
        g = self.graph_map[batch.padded_size]
        with decode_profile_range("FT.Graph.AttentionStaging"):
            self.attn_backend.prepare_for_replay(batch)
        with decode_profile_range("FT.Graph.Replay"):
            g.replay()
