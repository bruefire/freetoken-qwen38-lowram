...
def _qwen_fused_router_enabled() -> bool:
    """Default-on A/B switch; the token-count floor is applied separately."""
    return os.getenv("FREETOKEN_QWEN_FUSED_ROUTER", "1").strip().lower() not in _FALSE_VALUES


def _qwen_fused_router_min_tokens() -> int:
    """Auto-dispatch floor; the partial top-k kernel is fastest at batch=1."""
    raw = os.getenv("FREETOKEN_QWEN_FUSED_ROUTER_MIN_TOKENS", "1")
    try:
        return max(int(raw), 1)
    except ValueError as exc:
        raise ValueError(
            "FREETOKEN_QWEN_FUSED_ROUTER_MIN_TOKENS must be an integer"
        ) from exc


def _should_use_qwen_fused_router(gating_output: torch.Tensor, topk: int) -> bool:
    # Qwen3.8-Flash-Next is the only supported model with top-10 routing. Keep
    # power-of-two routers and other model-specific routing implementations unchanged.
    return (
        _qwen_fused_router_enabled()
        and gating_output.is_cuda
        and gating_output.dim() == 2
        and gating_output.stride(-1) == 1
        and gating_output.dtype in (torch.float16, torch.bfloat16)
        and gating_output.shape[0] >= _qwen_fused_router_min_tokens()
        and topk == 10
        and topk <= gating_output.shape[-1] <= 1024
    )
...
def fused_topk(
    ...
    if _should_use_qwen_fused_router(gating_output, topk):
        from freetoken.kernel.triton.qwen_router import qwen_fused_topk

        return qwen_fused_topk(
    ...
    if kernel_topk != topk:
        # triton_kernels returns the selected routes ordered by expert id, not
        # by score. A plain [:topk] would therefore keep the lowest expert ids
        # among the padded candidate set rather than the true top-k. Stable
        # score ordering also retains its smaller-id tie break.
        score_order = torch.argsort(
            topk_weights, dim=-1, descending=True, stable=True
        )[:, :topk]
        topk_weights = torch.gather(topk_weights, 1, score_order)
        topk_ids = torch.gather(topk_ids, 1, score_order)
        if renormalize:
            topk_weights = topk_weights / topk_weights.sum(dim=-1, keepdim=True)
