...
class OffloadMoeCache:
    ...
    def __post_init__(self) -> None:
        ...
        # kernel rewrites them to slots. Must be enabled before CUDA graph capture so the
        # device-side scatter is captured and re-run with each replay's routing.
    ...
    def decode_miss_stats_per_layer(self) -> dict:
        ...
        if self.decode_target == "hybrid":
            ...
            # Plain offload fetches every miss; only hybrid has a capped fetched subset.
            fetched = missing
        per_layer = []
